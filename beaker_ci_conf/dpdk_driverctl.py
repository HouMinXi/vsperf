    ...
    try:
        ...
        if 'vfio-pci' in S.getValue('TOOLS')['dpdk_modules']:
            ...
            tasks.run_task(['sudo', 'chmod', '-R', '666', '/dev/vfio/'],
                           ...
                           True)
            tasks.run_task(['sudo', 'driverctl', '-v', 'set-override'] + [_NICS_PCI[0]] + [_driver], _LOGGER,
                           'Binding NICs %s...' % _NICS_PCI,
                           True)
            tasks.run_task(['sudo', 'driverctl', '-v', 'set-override'] + [_NICS_PCI[1]] + [_driver], _LOGGER,
                           'Binding NICs %s...' % _NICS_PCI,
                            True)
    except subprocess.CalledProcessError:
...
def _unbind_nics():
    ...
    if not len(_NICS_PCI):
        ...
        return
    try:
        tasks.run_task(['sudo', 'driverctl', '-v', 'set-override'] + [_NICS_PCI[0]] + [S.getValue('BEAKER_NIC_DRIVER')], _LOGGER,
                           'Binding NICs %s...' % _NICS_PCI,
                           True)
        tasks.run_task(['sudo', 'driverctl', '-v', 'set-override'] + [_NICS_PCI[1]] + [S.getValue('BEAKER_NIC_DRIVER')], _LOGGER,
                           'Binding NICs %s...' % _NICS_PCI,
                            True)
    except subprocess.CalledProcessError:

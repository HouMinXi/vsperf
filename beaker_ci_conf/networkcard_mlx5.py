...
def get_infiniband_port_name(device):
    """ Returns infiniband device name for a kernel interface.

    param netdev: kernel interface name.
    :returns: string with assigned infiniband port.
    """
    port_path = "/sys/class/net/{}/dev_port".format(device)
    device_path = "/sys/class/net/{}/device/infiniband/".format(device)
    if os.path.isdir(device_path) and os.path.isfile(port_path):
        port = ""
        with open(port_path, 'r') as _file:
            port = _file.readline().rstrip('\n')
        try:
            return os.listdir(device_path)[0] + str(" port %s" %(int(port) + 1))
        except IndexError:
            return None
...
def get_mac(pci_handle):
    ...
    for mac in mac_path:
       if os.path.isfile(mac):
          with open(mac, 'r') as _file:
             macs.append(_file.readline().rstrip('\n'))

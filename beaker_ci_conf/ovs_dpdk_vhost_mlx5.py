...
class OvsDpdkVhost(IVSwitchOvs):
    ...
    def add_phy_port(self, switch_name):
        ...
        # MLX4 devices store infiniband port instead of pci device as a port name.
        if 'mlx4' in _nics[dpdk_count]['driver']:
            infiniband_port = networkcard.get_infiniband_port_name(_nics[dpdk_count]['device'])
            if infiniband_port:
                nic_pci = 'options:dpdk-devargs=' + infiniband_port
        ...
        if S.getValue('VSWITCH_JUMBO_FRAMES_ENABLED'):
            ...
            params += ['options:n_rxq_desc=1024,n_txq_desc=1024']

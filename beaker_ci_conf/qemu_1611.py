...
class IVnfQemu(IVnf):
    ...
    def _configure_testpmd(self):
        ...
        self.execute_and_wait('rpm -ivh /root/dpdkrpms/1611-2/dpdk*.rpm')
        ...
        self.execute_and_wait('./tools/dpdk*bind.py --status')
        '''
        self.execute_and_wait('/usr/share/dpdk/tools/dpdk-devbind.py --status')
        pci_list = ' '.join([nic['pci'] for nic in self._nics])
        self.execute_and_wait('/usr/share/dpdk/tools/dpdk-devbind.py -u ' + pci_list)
    ...
    def _bind_dpdk_driver(self, driver, pci_slots):
        ...
        if driver == 'uio_pci_generic':
            ...
            self.execute_and_wait('/usr/share/dpdk/tools/dpdk-devbind.py -b uio_pci_generic '+
                                  pci_slots)
            ...
            self.execute_and_wait('/usr/share/dpdk/tools/dpdk-devbind.py -b vfio-pci ' +
                                  pci_slots)
            ...
            self.execute_and_wait('insmod %s/kmod/igb_uio.ko' %
                                  S.getValue('RTE_TARGET'))
            self.execute_and_wait('/usr/share/dpdk/tools/dpdk-devbind.py -b igb_uio ' + pci_slots)
        else:
            self._logger.error(

...
class QemuPciPassthrough(IVnfQemu):
    ...
    def __init__(self):
        ...
        print(run_shell("/bin/bash /mnt/tests/kernel/networking/rt-kernel/vsperf/vsperf_CI/beaker_ci_conf/change_mac_qede.sh "+S.getValue('NIC1')+" "+S.getValue('NIC2')))
        ...
        for nic in self._host_nics:
            self._cmd += ['-device', 'vfio-pci,bus=root.'+str(i)+',host=' + nic['pci']]
            i=i+1
    ...
    def start(self):
        ...
        try:
            ...
            tasks.run_task(['sudo','/usr/bin/python2',S.getValue('TOOLS')['bind-tool'], '--bind=vfio-pci'] + nics_list,
    ...
    def stop(self):
        ...
        for nic in self._host_nics:
            if nic['driver']:
                try:
                    tasks.run_task(['sudo', '/usr/bin/python2',S.getValue('TOOLS')['bind-tool'], '--bind=' + nic['driver'], nic['pci']],

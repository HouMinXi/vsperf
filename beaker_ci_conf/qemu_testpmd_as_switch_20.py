...
class IVnfQemu(IVnf):
    ...
    def _configure_testpmd(self):
        ...
        self.execute_and_wait('ls /root/dpdkrpms/*.rpm|grep -w "dpdk-[0-9]*"| xargs rpm -ivh')
        self.execute_and_wait('ls /root/dpdkrpms/*.rpm|grep -w "dpdk-tools-[0-9]*"| xargs rpm -ivh')
    ...
    def _bind_dpdk_driver(self, driver, pci_slots):
        ...
        if driver == 'uio_pci_generic':
            ...
            self.execute_and_wait('driverctl -v set-override ' + '0000:' + pci_slots.split()[0] + ' vfio-pci ')
            self.execute_and_wait('driverctl -v set-override ' + '0000:' + pci_slots.split()[1] + ' vfio-pci ')
            ...
            self.execute_and_wait('insmod %s/kmod/igb_uio.ko' %
                                  S.getValue('RTE_TARGET'))
            self.execute_and_wait('/usr/share/dpdk/usertools/dpdk-devbind.py -b igb_uio ' + pci_slots)
        else:
            self._logger.error(

...
class Xena(ITrafficGenerator):
    ...
    def _create_throughput_result(root):
        ...
        if test_type == 'Throughput':
            ...
            try:
                ...
                # Stats for latency returned as N/A so just post them
                results[ResultsConstants.AVG_LATENCY_NS] = root[0][1][0][0][0].get(
                    'AvgLatency')
        elif test_type == 'Latency':
            results = OrderedDict()
            results[ResultsConstants.MIN_LATENCY_NS] = min(float(
                root[0][1][0][0][0].get('MinLatency')) * 1000, 
                float(root[0][1][0][0][1].get('MinLatency')) * 1000)
            results[ResultsConstants.MAX_LATENCY_NS] = max(float(
                root[0][1][0][0][0].get('MaxLatency')) * 1000,
                float(root[0][1][0][0][1].get('MaxLatency')) * 1000)
            results[ResultsConstants.AVG_LATENCY_NS] = (float(
                root[0][1][0][0][0].get('AvgLatency')) * 1000 +
                float(root[0][1][0][0][1].get('AvgLatency')) * 1000)/2
    ...
    def _setup_json_config(self, tests, loss_rate, testtype=None,
        ...
        try:
            ...
            elif testtype == '2544_latency':
                j_file.set_test_options_latency(
                    packet_sizes=self._params['traffic']['l2']['framesize'],
                    iterations=tests, loss_rate=loss_rate,
                    duration=self._duration,
                    micro_tpld=True if self._params[
                        'traffic']['l2']['framesize'] == 64 else False)
                j_file.enable_latency_test()
                j_file.disable_throughput_test()
                j_file.modify_latency_options(
                    settings.getValue('TRAFFICGEN_XENA_2544_LATENCY_START_VALUE'),
                    settings.getValue('TRAFFICGEN_XENA_2544_LATENCY_END_VALUE'),
                    settings.getValue('TRAFFICGEN_XENA_2544_LATENCY_STEP_VALUE')
                )
    ...
    def send_rfc2544_latency(self, traffic=None, tests=1, duration=20,
                                lossrate=0.0, latency=False):
        """Send traffic per RFC2544 throughput test specifications.
        ...
        self.start_rfc2544_latency(
            traffic, 1, self._duration, lossrate)
    ...
    def start_rfc2544_latency(self, traffic=None, tests=1, duration=20,
                                 lossrate=0.0):
        """Non-blocking version of 'send_rfc2544_latency'.
